...
## 4 Data Handling XC663119
## 5 Data Handling XC663119
...
install.packages("vegan") # Package with the species accumulation curve functions
...
setwd("D:/Bird Project/birdsong_output/XC663119_birdnet_output/XC663119_5_birdnet_output")

# Create a list object to store data frames

data_list <- list()

# Reading in data for XC663119 5 minute clips



file_name <- paste0("XC663119 - Soundscape_clip_1_30_clip_", i, "_5_analysis.xlsx")
...
for (i in 1:30) {
  ...
  S1[[i]] <- data_list[[i]] %>% count(common_name, sort = TRUE)
}
...
XC663119_5_wide_bird_matrix <- bind_rows(wide_list)

# Replacing NAs with 0

XC663119_5_wide_bird_matrix [is.na(XC663119_5_wide_bird_matrix)] <- 0

# Print to check

print(XC663119_5_wide_bird_matrix)


## 2.3 Species Accumulation Curve Cummulative

XC663119_curve_5 <- specaccum(XC663119_5_wide_bird_matrix, method = "random", permutations = 1000)

XC663119_curve_5

# For observed richness

XC663119_curve_5_or <- specaccum(XC663119_5_wide_bird_matrix, method = "collector", permutations = 1000)

XC663119_curve_5_or

plot(XC663119_curve_5_or, ci.type = "poly", col = "orchid", ci.col = "lavender", 
...
class(XC663119_curve_5)

# It is a specaccum object, so converting to a datadframe

specaccum_df_XC663119_5 <- data.frame(
  Sites = XC663119_curve_5$sites,
  Richness = XC663119_curve_5$richness,
  SD = XC663119_curve_5$sd
)

# Printing the dataframe: 

print(specaccum_df_XC663119_5)

# Calculating confidence intervals

specaccum_df_XC663119_5$lower_ci <- specaccum_df_XC663119_5$Richness - 1.96 * specaccum_df_XC663119_5$SD
specaccum_df_XC663119_5$upper_ci <- specaccum_df_XC663119_5$Richness + 1.96 * specaccum_df_XC663119_5$SD

# Making a prettier plot with ggplot

ggplot(specaccum_df_XC663119_5, aes(x = Sites, y = Richness)) +
  ...
  ggtitle("Species Accumulation Curve Based on Number of 5 Minute Samples for Site XC663119") + # Plot title
...
XC663119_5_wide_bird_matrix_df <- as.data.frame(XC663119_5_wide_bird_matrix)

# Add observation identifiers
XC663119_5_wide_bird_matrix_df <- XC663119_5_wide_bird_matrix_df %>%
  mutate(Observation = paste("Sample", 1:nrow(XC663119_5_wide_bird_matrix_df), sep = " "))

# Convert the dataframe to long format
XC663119_5_long_bird_matrix_df <- XC663119_5_wide_bird_matrix_df %>%
  pivot_longer(cols = -Observation, names_to = "Species", values_to = "Density")

# Replace periods with spaces in species names
XC663119_5_long_bird_matrix_df$Species <- gsub("\\.", " ", XC663119_5_long_bird_matrix_df$Species)

# Reorder the Observation factor to ensure correct order
XC663119_5_long_bird_matrix_df$Observation <- factor(XC663119_5_long_bird_matrix_df$Observation, levels = unique(XC663119_5_long_bird_matrix_df$Observation))

# Choose a color palette from RColorBrewer
palette <- brewer.pal(n = 9, name = "PuBuGn")

# Create the heatmap using ggplot2
ggplot(XC663119_5_long_bird_matrix_df, aes(x = Species, y = Observation, fill = Density)) +
  ...
  labs(title = "Species Density Heatmap for XC663119 ", x = "Species", y = "Sample", fill = "Density")
...
# Filter rows for XC663119

# Filter rows where site is "XC663119" and create a new data frame
XC663119_gt_df <- gt_df %>%
  filter(Reference == "XC663119")

print(XC663119_gt_df)

print(XC663119_5_long_bird_matrix_df)
...
presence_absence_density <- XC663119_5_long_bird_matrix_df %>%
  mutate(Presence = ifelse(Density > 0, 1, 0)) %>%
  select(Species, Presence)

# Create species reference presence-absence matrix for XC668910_gt_df
presence_absence_reference <- XC663119_gt_df %>%
  mutate(Presence = 1) %>%
...
ggplot(presence_absence_combined_long, aes(x = Source, y = Species, fill = factor(Presence))) +
  ...
  labs(title = "Presence-Absence Heatmap for XC663119", x = "Source", y = "Species")
...
species_density <- XC663119_5_long_bird_matrix_df %>%
  group_by(Species) %>%
...
XC663119_5_long_bird_matrix_df$Species <- factor(XC663119_5_long_bird_matrix_df$Species,
                                                 levels = species_density$Species)

# Reorder the Observation factor to ensure it remains in the original order
XC663119_5_long_bird_matrix_df$Observation <- factor(XC663119_5_long_bird_matrix_df$Observation,
                                                     levels = unique(XC663119_5_long_bird_matrix_df$Observation))
